%{
Examine Laminar Profile of Lateralized Visuomotor Activity in SEF
2023-02-12 | S P Errington
%}
...
%% Curate: Define data tables
% Create different data tables to reference for plots
...
% Negative LSI
timedepth_table_lateral_R = timedepth_table(timedepth_table.ls_flag == 1 &...
    timedepth_table.ls_index < 0,:);

% Non-significant LSI
timedepth_table_lateral_nonSig = timedepth_table(timedepth_table.ls_flag == 0,:);
...
%% Plot: Generate time-depth plots

% Define datatable to plot
input_table = []; input_table = timedepth_table_lateral_L;


for neuronlist_i = 1:size(input_table,1) 
    % Get the depth
    depth = input_table.depth_ch(neuronlist_i);
    
    % Onset and offset times of the modulation aligned on target and
    % saccade
    onset_target = input_table.mod_onset_target(neuronlist_i);
    offset_target = input_table.mod_offset_target(neuronlist_i);
    onset_saccade = input_table.mod_onset_saccade(neuronlist_i);
    offset_saccade = input_table.mod_offset_saccade(neuronlist_i);    
...
scatter(input_table.mod_onset_target(input_table.spkwidth > 249),...
    input_table.depth_ch(input_table.spkwidth > 249),'^','filled','MarkerEdgeColor','black','MarkerFacecolor','white')
% Add markers for narrow-spike neurons (those <250 us spk width)
scatter(input_table.mod_onset_target(input_table.spkwidth < 249),...
    input_table.depth_ch(input_table.spkwidth < 249),60,'h','filled','MarkerEdgeColor','black','MarkerFacecolor','white')
