...
parfor neuron_i = 1:575
    ...
    for ssd_i = 1:ssd_n
        ...
        if fr_saccade_baseline_std == 0 | executiveBeh.inh_trcount_SSD{session_i}(ssd_i) < 10
            ...
            sdf_saccade_canc_ssd_raw(ssd_i,:) = nan(1,length(alignment_index_canc));
            ...
            sdf_saccade_canc_ssd_zscore(ssd_i,:) = nan(1,length(alignment_index_canc));
            
        else
            sdf_saccade_ns_fast_ssd_raw(ssd_i,:) = nanmean(spk_in.SDF.saccade(trial_ns_fast,:));
            sdf_saccade_ns_slow_ssd_raw(ssd_i,:) = nanmean(spk_in.SDF.saccade(trial_ns_slow,:));
            sdf_saccade_nc_ssd_raw(ssd_i,:) = nanmean(spk_in.SDF.saccade(trial_nc,:));
            sdf_saccade_canc_ssd_raw(ssd_i,:) = nanmean(spk_in.SDF.target(trial_c,alignment_index_canc));
    ...
    sdf_saccade_ns_slow_raw(neuron_i,:) = nanmean(sdf_saccade_ns_fast_ssd_raw);
    sdf_saccade_nc_raw(neuron_i,:)      = nanmean(sdf_saccade_ns_fast_ssd_raw);
    sdf_saccade_canc_raw(neuron_i,:)    = nanmean(sdf_saccade_ns_fast_ssd_raw);
...
for page_i = 1:n_batches
    ...
    for plot_i = 1:n_plot_sheet
        ...
        try
            ...
            plot(timewins.sdf, sdf_saccade_ns_fast_zscore(neuron_i,:),'color',colors.nostop)
            plot(timewins.sdf, sdf_saccade_ns_slow_zscore(neuron_i,:),'color',colors.nostop,'LineWidth',2)
            plot(timewins.sdf, sdf_saccade_nc_zscore(neuron_i,:),'color',colors.noncanc)
            plot([-600:600], sdf_saccade_canc_zscore(neuron_i,:),'color',colors.canceled)
            
            xlim([-400 400]); vline(0,'k--'); hline(0,'k--')%, hline([-3 3],'r:'); hline([-6 6],'r:'); 
...
for neuron_i = 1:575
    neuron_sdf = []; neuron_sdf = sdf_saccade_zscore(neuron_i,:);
...
for neuron_i = 1:575
    ...
    visual_flag = fr_comp_saccade_sig_h(neuron_i);
    visual_fac = fr_comp_saccade_sig_dir(neuron_i) == 1;
    visual_sup = fr_comp_saccade_sig_dir(neuron_i) == 0;
...
for page_i = 1:n_batches
    ...
    for plot_i = 1:n_plot_sheet
        ...
        try
            ...
            plot(timewins.sdf, sdf_saccade_zscore(neuron_i,:),'k');
    ...
    filename = fullfile(dirs.root,'results','sdf_saccade_figs',['sdf_target_overview_pg' int2str(page_i) '.pdf']);
    set(fig_out,'PaperSize',[20 10]); %set the paper size to what you want
    print(fig_out,filename,'-dpdf') % then print it
    close(fig_out)
end
...
save(fullfile(dirs.root,'results','mat_files','visual_sdf_1.mat'),'sdf_saccade_raw','sdf_saccade_zscore','sdf_saccade_maxnorm')
